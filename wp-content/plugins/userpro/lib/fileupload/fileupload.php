	...
	if (empty($_FILES["userpro_file"]["name"])){
		...
		if ($_FILES["userpro_file"]["error"] > 0){
			...
			if(!is_uploaded_file($_FILES["userpro_file"]["tmp_name"])){
				...
				$accepted_file_mime_types = array('image/gif','image/jpg','image/jpeg','image/png','application/pdf','application/zip','application/vnd.openxmlformats-officedocument.wordprocessingml.document','application/msword','text/plain','audio/wav','audio/mp3','audio/mp4','video/mp4');
				...
				if( !in_array($file_extension, array( '.gif','.jpg','.jpeg','.png','.pdf','.txt','.zip','.doc','.docx','.wav','.mp3','.mp4'  )  ) || !in_array($fileinfo,$accepted_file_mime_types) ){
                	$ret['status'] = 0;
                	$ret['status2'] = $file_extension;
                	$ret['status3'] = $fileinfo;
